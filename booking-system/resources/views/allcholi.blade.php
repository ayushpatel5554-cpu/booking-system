...
<body class="bg-gray-50 min-h-screen">
    ...
    <aside class="sidebar fixed top-0 left-0 w-sidebar h-screen bg-sidebar-bg text-white p-6 hidden lg:flex flex-col">
        <div class="logo text-center mb-0 ">
            <img class=" w-100 h-100 -mt-10 mx-auto object-contain"
        ...
        <ul class="flex flex-col space-y-2">
            <li>
                <a class="flex items-center p-3 rounded-md transition duration-150" href="/dashboard">
                    <i class="fas fa-tachometer-alt fa-fw mr-3"></i> Dashboard
                </a>
            </li>
            <li>
                <a class="text-white hover:text-accent-color flex items-center p-3 rounded-md transition duration-150"
                    href="/admin/totalbooking">
                    <i class="fas fa-calendar-check fa-fw mr-3"></i> Total Booking
                </a>
            </li>
            <li>
                <a class="nav-link-active flex items-center p-3 rounded-md transition duration-150"
                    href="/admin/allcholi">
    ...
    <main class="main-content p-0 lg:p-10">
        ...
        <header class="flex justify-between items-center mb-6 p-4 md:p-0 border-b pb-4">
            <h1 class="md:text-2xl text-xl font-semibold text-gray-800">All Choli For Rent</h1>
        ...
        <div class="bg-white shadow-lg rounded-lg">
            <div class="p-4 border-b text-lg font-semibold text-gray-800 flex flex-col sm:flex-row justify-between items-start sm:items-center space-y-3 sm:space-y-0">
                ...
                <div class="leading-snug">
                    <span class="text-base sm:text-lg block font-semibold">All Rented Cholis</span>

                    <span class="text-sm sm:text-base font-normal block text-gray-600">બધી ભાડાની ચોલીઓ</span>
                </div>

                <button onclick="openAddModal()"
                    class="w-full sm:w-auto bg-green-500 hover:bg-green-600 text-white font-bold py-2 px-4 rounded transition duration-150 shadow-md flex items-center justify-center sm:justify-start text-base">
                    <i class="fas fa-plus mr-2"></i> Add New Choli
                </button>
            </div>
        </div>
        ...
        <div class="p-4 overflow-x-auto">
            <table class="min-w-full table-fixed divide-y divide-gray-200">
                ...
                <tbody id="table-body" class="bg-white divide-y divide-gray-200">
                    @foreach ($cholis as $choli)
                    <tr class="table-row h-16" data-choli-id="{{ $choli->choli_no }}">
                        <td class="px-4 py-3 text-sm font-medium text-gray-900 text-center">{{ $choli->choli_no }}</td>
                        ...
                        <td class="px-4 py-3 text-sm font-medium text-gray-900 text-center">{{ $choli->choli_name }}</td>

                        <td class="px-4 py-3 text-sm text-gray-900 text-center">₹{{ $choli->rent_price }}</td>
            ...
            choli_name: '{{ $choli->choli_name }}',
            rent_price: '{{ $choli->rent_price }}',
            photo_url: '{{ asset('storage/' . $choli->photo) }}'
        })"
                                class="text-indigo-600 hover:text-indigo-900 transition duration-150 mr-2"
                                title="Edit Choli {{ $choli->choli_no }}">
                                <i class="fas fa-edit"></i> Edit
                            </button>



                            <form action="{{ route('choli.destroy', $choli->id) }}" method="POST" class="inline">
                                @csrf
                                @method('DELETE')
                                <button type="submit"
        ...
        <div id="addCholiModal" class="fixed inset-0 bg-black bg-opacity-70 flex items-center justify-center hidden z-50">
            <div class="relative bg-white p-6 rounded-lg max-w-lg w-full shadow-2xl">
                ...
                <form id="add-choli-form" action="{{ route('choli.store') }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    <div class="mb-4">
                    ...
                    <div class="mb-6">
                        <label for="choli_photo" class="block text-sm font-medium text-gray-700">Choli Photo / ચોલી ફોટો</label>
                        <input type="file" id="choli_photo" name="choli_photo" accept="image/*" required
        ...
        <div id="updateCholiModal" class="fixed inset-0 bg-black bg-opacity-70 flex items-center justify-center hidden z-50">
            <div class="relative bg-white p-6 rounded-lg max-w-lg w-full shadow-2xl">
                ...
                <form id="update-choli-form" method="POST" enctype="multipart/form-data">
                    ...
                    <input type="hidden" id="update-choli-id" name="choli_id">

                    <div class="mb-4">
                        <label for="update-choli-no" class="block text-sm font-medium text-gray-700">Choli No. / ચોલી નં.</label>
                        <input type="text" id="update-choli-no" name="choli_no" required
                            class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-accent-color focus:border-accent-color sm:text-sm">
                    </div>

                    <div class="mb-4">
                        <label for="update-choli-name" class="block text-sm font-medium text-gray-700">Choli Name / ચોલી નામ</label>
                        <input type="text" id="update-choli-name" name="choli_name" required
                            class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-accent-color focus:border-accent-color sm:text-sm">
                    </div>

                    <div class="mb-4">
                        <label for="update-rent-price" class="block text-sm font-medium text-gray-700">Rent Price (₹) / ભાડું (₹)</label>
                        <input type="number" id="update-rent-price" name="rent_price" required min="100"
                            class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-accent-color focus:border-accent-color sm:text-sm">
                    </div>

                    <div class="mb-6">
                        <label for="update-choli-photo" class="block text-sm font-medium text-gray-700">Choli Photo / ચોલી ફોટો</label>
                        <img id="update-choli-photo-preview" class="w-24 h-24 object-cover mb-2 rounded">
                        <input type="file" id="update-choli-photo" name="photo" accept="image/*"
                            class="mt-1 block w-full text-gray-700 border border-gray-300 rounded-md shadow-sm p-1">
                    </div>
                    ...
                    <div class="flex justify-end space-x-3">
                        ...
                        <button type="submit"
                            class="py-2 px-4 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-rental-dark hover:bg-rental-dark/90 transition duration-150">
                            <i class="fas fa-save mr-2"></i> Update Choli
                        </button>
                    </div>
                </form>
            </div>
    ...
    <script>
        function openUpdateModal(choli) {
            ...
            form.action = `/choli/${choli.id}`;

            // Show modal
            document.getElementById('updateCholiModal').classList.remove('hidden');
        }
    ...
    <script>
        const addCholiModal = document.getElementById('addCholiModal');

        function openAddModal() {
            addCholiModal.classList.remove('hidden');
        }

        function closeAddModal() {
            addCholiModal.classList.add('hidden');
            // Optional: Clear the form when closing
            document.getElementById('add-choli-form').reset();
        }

        // Optional: Close modal when clicking outside the form content
        addCholiModal.addEventListener('click', function(e) {
            if (e.target === addCholiModal) {
                closeAddModal();
            }
        });

        // Placeholder functions for action buttons (replace with your actual logic)
        function handleUpdate(choliId) {
            toastr.info('Simulating update for Choli ID: ' + choliId);
            // Example: redirect to an edit page or open an edit modal
        }

        function handleDelete(choliId) {
            toastr.error('Simulating delete for Choli ID: ' + choliId, 'Confirm Deletion');
            // Example: show a confirmation dialog and then send a delete request
        }
    </script>
</body>
